...
var BRS = (function(BRS, $, undefined) {
    BRS.automaticallyCheckRecipient = function() {
        ...
        $recipientFields.on("checkRecipient", function() {
            ...
            if (value && value != "ALITA-____-____-____-_____") {
            ...
            if (response.publicKey) {
                ...
                if (response.errorCode) {
                    if (response.errorCode === 4) {
                        callback({
                            "type": "danger",
                            "message": $.t("recipient_malformed") + (!/^(ALITA\-)/i.test(accountId) ? " " + $.t("recipient_alias_suggestion") : ""),
                            "account": null
                        });
    ...
    BRS.checkRecipient = function(account, modal) {
        ...
        if (/^(ALITA\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {
