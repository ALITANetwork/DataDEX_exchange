...
var BRS = (function(BRS, $, undefined) {
    ...
    $('#send_money_modal').on('hide.bs.modal', function (e) {
           $("#total_amount_multi_out").html('0.1 ALITA');
        });
  ...
  var accountButton;
        ...
        switch (transaction.subtype) {
			...
			case 1:
				...
				for (var i = 0; i < transaction.attachment.recipients.length; i++) {
					...
					var amount = BRS.formatAmount(recipient[1]) + " ALITA";
					if (i !== 0) {
						recipientHtml += "<br />";
					}
			...
			case 2:
                ...
                data = {
                    ...
                    "amount_formatted_html": BRS.formatAmount(transaction.amountNQT) + " ALITA (" + BRS.formatAmount(amountEach.toString()) + " ALITA for each recipient)",
                    "fee": transaction.feeNQT,
                    "recipient_formatted_html": recipientHtml,
                    "sender": BRS.getAccountTitle(transaction, "sender")
                };
    ...
    var type;
		...
		BRS.sendRequest("getAsset", {
			...
			"asset_name": asset.name,
			"quantity": [transaction.attachment.quantityQNT, asset.decimals],
			"price_formatted_html": BRS.formatOrderPricePerWholeQNT(transaction.attachment.priceNQT, asset.decimals) + " ALITA",
			"total_formatted_html": BRS.formatAmount(BRS.calculateOrderTotalNQT(transaction.attachment.quantityQNT, transaction.attachment.priceNQT)) + " ALITA"
		    };
		...
		BRS.sendRequest("getDGSGood", {
			...
			"item_name": goods.name,
			"new_price_formatted_html": BRS.formatAmount(transaction.attachment.priceNQT) + " ALITA",
			"seller": BRS.getAccountFormatted(goods, "seller")
		    };
		...
		BRS.sendRequest("getDGSPurchase", {
			...
			if (purchase.quantity != "1") {
			    ...
			    data.total_formatted_html = orderTotal + " ALITA";
			}
		...
		BRS.sendRequest("getDGSPurchase", {
			...
			data.order_total_formatted_html = BRS.formatAmount(orderTotal) + " ALITA";
