var NSV = (function(NSV, $, undefined) {
    ...
    NSV.div_ad_gen = function () {
        ...
        if (BRS.accountInfo.unconfirmedBalanceNQT) {
            ...
            if (balance < len) {
                err_message = "You don't have enough ALITA in this account for this distribution. ".concat(String(len),"ALITA needed.");
            }
        }
        else {
    ...
    NSV.getAccountError = function(accountId, callback) {
        BRS.sendRequest("getAccount", {
            ...
            if (response.publicKey) {
                callback({
                    "type": "info",
                    "message": "The recipient account has a public key and a balance of " + BRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " ALITA.",
                    "account": response
                });
            }
            else {
                if (response.errorCode) {
                    if (response.errorCode == 4) {
                        callback({
                            "type": "danger",
                            "message": "The recipient account is malformed, please adjust." + (!(/^(ALITA\-)/i.test(accountId)) ? " If you want to type an alias, prepend it with the @ character." : ""),
                            "account": null
                        });
                    }
                    ...
                    callback({
                        "type": "warning",
                        "message": "The recipient account does not have a public key, meaning it has never had an outgoing transaction. The account has a balance of " + BRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " ALITA. Please double check your recipient address before submitting.",
                        "account": response
                    });
                }
            }
    ...
    NSV.div_send_calc_dividends = function () {
        ...
        if (!(/^\d+$/.test(asset))) {
            ...
            BRS.sendOutsideRequest("/burst?requestType=" + "getAsset", {
                ...
                if (NSV_div_send_out_burst) {
                    output = output.concat("Summary of proposed distribution of  ", amount, "ALITA to ", String(aa_len),"\n");
                }
                else {
        ...
        if (NSV_div_send_out_burst) {
            ...
            if (BRS.accountInfo.unconfirmedBalanceNQT) {
                ...
                if (balance < total_cost) {
                    err_message = "You don't have enough Burst in this account for this distribution. ".concat(String(len),"ALITA needed.");
                }
            }
            else {
                err_message = "This account doesn't seem to have any funds.";
            ...
            if (BRS.accountInfo.unconfirmedBalanceNQT) {
                ...
                if (balance < (len*100000000)) {
                    err_message = "You don't have enough Burst in this account to pay the fees for this distribution. ".concat(String(len),"ALITA needed.");
                }
            }
            else {
                err_message = "This account doesn't seem to have any funds.";
                                ...
                                if (response.timestamp <= final_time) {
                                    if (cur_asset == response.attachment.asset) {
                                        ...
                                        if (response.senderRS == cur_account) {
                                            NSV_div_send_acc_array[cur_index].amount -= parseInt(response.attachment.quantityQNT,10);
                                            if (response.recipientRS != "ALITA-NU58-Z4QR-XXKE-94DHH") {	//genesis (86281612813136630310)
    ...
    NSV.shareswap_calc = function() {
        ...
        if (NSV_redeemed_assets.length === 0) {
            ...
            if (num_tobe_swapped > 0) {
                out_message = out_message + "ALITA needed for transactions: " + num_tobe_swapped.toString() + ", Replacement Assets needed: " + total_replacement_assets.toString() + "\n";
