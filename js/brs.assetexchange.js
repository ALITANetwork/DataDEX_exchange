...
var BRS = (function (BRS, $, undefined) {
    ...
    BRS.forms.addAssetBookmark = function ($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^ALITA\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^ALITA\-/i.test(data.id)) {
    ...
    // #buy_asset_price
    $("#sell_asset_quantity, #sell_asset_price, #buy_asset_quantity").keyup(function (e) {
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        $("#asset_order_total").html(totalNXT + " ALITA");
        $("#asset_order_fee_paid").html(BRS.formatAmount(feeNQT) + " ALITA");
