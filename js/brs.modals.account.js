...
var BRS = (function(BRS, $, undefined) {
	...
	if (account.unconfirmedBalanceNQT == "0") {
	    ...
	    $("#user_info_modal_account_balance").html(BRS.formatAmount(account.unconfirmedBalanceNQT) + " ALITA");
	}
    ...
    var receiving;
		for (var i = 0; i < response.transactions.length; i++) {
		    ...
		    if (/^ALITA\-/i.test(BRS.userInfoModal.user)) {
	...
	BRS.sendRequest("getDGSGoods", {
		...
		for (var i = 0; i < response.goods.length; i++) {
		    ...
		    rows += "<tr><td><a href='#' data-goto-goods='" + String(good.goods).escapeHTML() + "' data-seller='" + String(BRS.userInfoModal.user).escapeHTML() + "'>" + String(good.name).escapeHTML() + "</a></td><td>" + BRS.formatAmount(good.priceNQT) + " ALITA</td><td>" + BRS.format(good.quantity) + "</td></tr>";
		}
	    }
