...
var BRS = (function (BRS, $, undefined) {
    ...
    BRS.submitForm = function ($modal, $btn) {
        ...
        if (data.recipient) {
            ...
            if (/^\d+$/.test(data.recipient)) {} else if (!/^ALITA\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(data.recipient)) {
                var convertedAccountId = $modal.find("input[name=converted_account_id]").val();
                if (!convertedAccountId || (!/^\d+$/.test(convertedAccountId) && !/^ALITA\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(convertedAccountId))) {
