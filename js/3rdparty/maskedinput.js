...
! function($) {
			...
			return !mask && this.length > 0 ? (input = $(this[0]), input.data($.mask.dataName)()) : (settings = $.extend({
				...
				firstNonMaskPos = null, $.each(mask.split(""), function(i, c) {
					...
					function keydownEvent(e) {
						...
						if (e.keyCode == 8) {
							...
							if (settings.unmask !== false) {
								//backspace, remove
								if ((pos.begin == 0 && pos.end == 24) || (currentInput == "ALITA-____-____-____-_____" && pos.begin == 6)) {
									input.val("");
									$(this).trigger("unmask");
									return;
								}
					...
					function keypressEvent(e) {
						...
						if (/^ALITA\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(input.val())) {
					...
					function checkVal(allow) {
						input.val(input.val().replace(/^\s*ALITA\-\s*ALITA/i, "ALITA-"));
					...
					if (settings.noMask) {
						input.bind("keyup.remask", function(e) {
							if (input.val().toLowerCase() == "burst-") {
								input.val("").mask("ALITA-****-****-****-*****").unbind(".remask").trigger("focus");
							}
						}).bind("paste.remask", function(e) {
							setTimeout(function() {
								var newInput = input.val();

								if (/^ALITA\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(newInput) || /^ALITA[A-Z0-9]{17}/i.test(newInput)) {
									input.mask("ALITA-****-****-****-*****").trigger("checkRecipient").unbind(".remask");
								}
							}, 0);
						});
						return;
					...
					input.data($.mask.dataName, function() {
						...
						setTimeout(function() {
							...
							if (/^ALITA\-[0-9]{19,20}$/i.test(pasted)) {
								//old style accounts..
								input.val("").trigger("oldRecipientPaste");
							} else {
								var match = /^ALITA\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.exec(pasted);

								if (match && match[0]) {
									input.val(match[0]).trigger("checkRecipient");
								} else {
									match = /^ALITA[A-Z0-9]{17}/i.exec(pasted);
									if (match && match[0]) {
	...
	function text_diff(first, second) {
		...
		if (/^ALITA\-/i.test(second) && !/^ALITA\-/i.test(diff)) {
			diff = "ALITA-" + diff;
		}
